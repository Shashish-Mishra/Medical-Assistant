...
def main():
    ...
    def scrap():
        ...
        lst=[]
        lbl=[]
        dt=r.request('get','https://www.1mg.com/search/all?name=%s'%(e1.get()),headers=headers)
        print(dt)
            ...
            if i.get('class'):
                if len([x for x in i.get('class') if 'style__container__' in x])>0:
                    if i.find('a'):
                        x=i.find('a')
                        try:
                            dts=r.request('get','https://www.1mg.com'+x.get('href'),headers=headers)
